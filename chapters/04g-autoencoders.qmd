...
```{julia}
opt = Adam(0.003f0)
tstate = Training.TrainState(autoencoder, ps, st, opt)

for epoch in 1:500
    _, loss, _, tstate = Training.single_train_step!(
        AutoZygote(), ae_loss, (X_train, Y_train), tstate
    )
...
- **Subsurface modeling** — @lopez2019emulator used deep autoencoder-based approaches for inverse modeling of subsurface transport, where the autoencoder compresses the parameter space before inversion.
- **Overview** — @bergen2019mlgeo discusses the role of representation learning and dimensionality reduction, of which autoencoders are a central tool, across geoscience disciplines.
