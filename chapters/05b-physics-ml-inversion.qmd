...
$$
\mathcal{L}(\theta) = \underbrace{\frac{1}{N_d}\sum_{i=1}^{N_d}\bigl(d_i^{\text{pred}}(\theta) - d_i^{\text{obs}}\bigr)^2}_{\text{data misfit}} + \underbrace{\lambda_r \frac{1}{N_c}\sum_{j=1}^{N_c}\bigl|\mathcal{N}[u; m_\theta](\mathbf{x}_j)\bigr|^2}_{\text{PDE residual}}
$$
...
```{julia}
# INR for the diffusion coefficient κ(x)
# Fourier feature encoding + MLP
n_features = 32
...
# κ-network: Fourier features → MLP → positive κ
κ_model = Chain(
...
# u-network: also an INR for the solution field
u_model = Chain(
    Dense(2 * n_features => 64, tanh),
    Dense(64 => 64, tanh),
    Dense(64 => 1)
...
```{julia}
# Combined loss: data misfit + PDE residual
function inversion_loss(ps_κ, ps_u, st_κ, st_u)
    ...
    total = loss_data + 1.0f0 * loss_pde + 10.0f0 * loss_bc
    return total
end
```
